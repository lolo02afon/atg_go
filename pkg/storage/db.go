...
import (
	...
	pq "github.com/lib/pq" // Работа с типами PostgreSQL, например массивами
)
...
func (db *DB) CreateAccount(account models.Account) (*models.Account, error) {
	...
	// Если полы не указаны, добавляем значение по умолчанию
	...
	err := db.Conn.QueryRow(
		...
		account.ProxyID,
		pq.Array(gender), // Передаём массив полов
	).Scan(&account.ID)
...
func (db *DB) GetAccountByID(id int) (*models.Account, error) {
	...
	err := db.Conn.QueryRow(query, id).Scan(
		...
		pq.Array(&account.Gender), // Читаем массив полов из БД
		&account.ProxyID,
		&account.OrderID,
		&proxyID,
		&proxyIP,
