...
// increaseAccountsSessionsDisconnect увеличивает счётчик в таблице accounts_sessions_disconnect.
...
func (db *DB) increaseAccountsSessionsDisconnect(column string) error {
	...
	res, err := db.Conn.Exec(
		"UPDATE accounts_sessions_disconnect SET " + column + " = " + column + " + 1, date_time = NOW() WHERE id = 1",
	)
	if err != nil {
		return err
	}
	...
	if rows == 0 {
		...
		_, err = db.Conn.Exec("INSERT INTO accounts_sessions_disconnect (id, " + column + ") VALUES (1, 1)")
	}
	return err
}
