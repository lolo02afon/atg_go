...
func SendReaction(phone, channelURL string, apiID int, apiHash string, msgCount int, userIDs []int) (int, error) {
	...
	err = client.Run(ctx, func(ctx context.Context) error {
		...
		// Получаем последние сообщения из обсуждения (чата) целиком,
		// повторно используя универсальную функцию получения истории канала.
		if msgCount < 1 {
			msgCount = 1
		}
		messages, err := module.GetChannelPosts(ctx, api, discussion.Chat, msgCount)
		if err != nil {
			return fmt.Errorf("не удалось получить историю обсуждения: %w", err)
		}
		...
		for i := len(messages) - 1; i >= 0; i-- {
			...
			if msg.Reactions == nil || len(msg.Reactions.Results) == 0 {
